      ...
      <div className="sticky top-0 flex h-screen items-center overflow-hidden">
        <motion.div style={{ x }} className="flex gap-10 px-4 md:px-20">
          {items.map((item) => (
            ...
            >
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent opacity-50 z-0"></div>
              <div className="relative z-10">
                <h1 className="text-6xl md:text-[8vw] font-black text-white/30 group-hover:text-white transition-colors duration-700 uppercase tracking-tighter leading-none">
                  {item.title}
                </h1>
                <p className="text-xl md:text-3xl text-neutral-400 mt-6 max-w-xl">{item.subtitle}</p>
              </div>
            </div>
          ))}
        </motion.div>
